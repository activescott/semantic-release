...
module.exports = async ({cwd, env, branch, options: {repositoryUrl}}) => {
  ...
  try {
    ...
    console.log("get-git-auth-url: verifyAuth failed. Looking for GIT_TOKENS environment variables...")
    const envVar = Object.keys(GIT_TOKENS).find(envVar => !isNil(env[envVar]));
    console.log(`get-git-auth-url: found environment variable `${envVar}` with value 'env[envVar]'.`)
